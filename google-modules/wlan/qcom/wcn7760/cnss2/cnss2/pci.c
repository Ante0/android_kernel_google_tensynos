...
#else
		/*
		 * If this failure happens during a link-down recovery, do not panic the system.
		 * Just let it power off and fail silently.
		 * 5% probability to trigger CNSS_ASSERT for ramdump debugging.
		 */
		if (test_bit(CNSS_DRIVER_RECOVERY, &plat_priv->driver_state) &&
		    pci_priv->pci_link_down_ind) {
			if (get_random_u32() % 100 < 5) {
				cnss_pr_err("PCIe link remains down during "
					    "recovery, trigger assert "
					    "(5%% probability)\n");
				CNSS_ASSERT(0);
			} else {
				cnss_pr_err("PCIe link remains down during "
					    "recovery, skip assert\n");
			}
		} else {
			CNSS_ASSERT(0);
		}
...
int cnss_pci_resume_bus(struct cnss_pci_data *pci_priv)
{
...
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
	{
		...
		if (device_id != pci_priv->device_id) {
			cnss_fatal_err("PCIe link not up after RC PM resume (id=0x%x)\n",
					device_id);
			cnss_pci_link_down(&pci_dev->dev);
			ret = -EAGAIN;
			goto out;
...
static int cnss_pci_enable_msi(struct cnss_pci_data *pci_priv)
{
	...
	case COLOGNE_DEVICE_ID:
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
		msi_pos = pci_find_capability(pci_dev, PCI_CAP_ID_MSI);
		pci_dev->no_64bit_msi = 0;
		if (msi_pos) {
			cnss_pr_info("program 64-bit MSI enable\n");
			pci_read_config_dword(pci_dev, msi_pos + PCI_MSI_FLAGS, &val);
			val |= PCI_MSI_FLAGS_64BIT;
			pci_write_config_dword(pci_dev, msi_pos + PCI_MSI_FLAGS, val);
			pci_read_config_dword(pci_dev, msi_pos + PCI_MSI_FLAGS, &val);
			cnss_pr_info("val = 0x%x pci_dev->no_64bit_msi = %d\n",
				val, pci_dev->no_64bit_msi);
		} else {
			cnss_pr_err("MSI capability not found!\n");
		}
#endif
		num_vectors = pci_alloc_irq_vectors(pci_dev,
						    msi_config->total_vectors,
				  ...
				  void *va, dma_addr_t dma, size_t size)
{
	...
	if (pci_priv->iommu_domain)
		pa = iommu_iova_to_phys(pci_priv->iommu_domain, dma);

	if (pa)
		cnss_pr_dbg("Seg: %x, va: %pK, dma: %pa, pa: %pa, size: 0x%zx\n",
			    seg_no, va, &dma, &pa, size);
	else
...
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
static void cnss_pci_wake_work_hdlr(struct work_struct *work)
{
	struct cnss_pci_data *pci_priv =
		container_of(work, struct cnss_pci_data, wake_work);
	int ret;

	cnss_pr_dbg("WLAN PCI wake work handler started\n");
	ret = cnss_pci_pm_runtime_resume(pci_priv);
	if (ret < 0) {
		cnss_pr_err("Failed to resume device synchronously in wake work: %d\n",
			ret);
		cnss_schedule_recovery(&pci_priv->pci_dev->dev,
...
static irqreturn_t cnss_pci_wake_handler(int irq, void *data)
{
	...
	if ((cnss_pci_get_monitor_wake_intr(pci_priv) &&
		...
		cnss_pci_set_monitor_wake_intr(pci_priv, false);
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
		queue_pm_work(&pci_priv->wake_work);
#else
...
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
		INIT_WORK(&pci_priv->wake_work, cnss_pci_wake_work_hdlr);
#endif
		cnss_pci_get_link_status(pci_priv);
...
#if IS_ENABLED(CONFIG_WCN_GOOGLE)
	cancel_work_sync(&pci_priv->wake_work);
#endif
