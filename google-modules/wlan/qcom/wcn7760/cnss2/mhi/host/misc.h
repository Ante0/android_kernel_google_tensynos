...
#ifdef CONFIG_MHI_OUT_OF_TREE

#define MHI_VERB(dev, fmt, ...)                       \
	do {                                              \
		if (dev) {                                    \
			dev_get_drvdata(&mhi_cntrl->mhi_dev->dev);  \
			printk("[mhi:%s]" fmt, __func__, ##__VA_ARGS__); \
		}                                             \
	} while (0)

#define MHI_LOG(dev, fmt, ...)                        \
	do {                                              \
		if (dev) {                                    \
			dev_get_drvdata(&mhi_cntrl->mhi_dev->dev);  \
			printk("[mhi:%s]" fmt, __func__, ##__VA_ARGS__); \
