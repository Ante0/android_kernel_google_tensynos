...
s32
wl_handle_usd_tx_complete(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
		const wl_event_msg_t *e, u32 packetID)
{
	s32 ret = BCME_OK;
	struct net_device *ndev = NULL;
	const u8 *buf = NULL;
		...
		if (packetID == cfg->afx_hdl->usd_pub_tx_cookie) {
			if (status == WLC_E_STATUS_SUCCESS) {
				...
				if (status == WLC_E_STATUS_SUPPRESS) {
					CFGP2P_ACTION(("USD TX actfrm : SUPPRES\n"));
				} else {
	...
	} else if (event_type == WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE) {
		WL_INFORM(("send tx expiry status :%d \n",
			cfg->afx_hdl->usd_pub_tx_cookie));
		wl_clr_drv_status(cfg, USD_PUB_ACTIVE, ndev);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
		cfg80211_tx_mgmt_expired(cfgdev, cfg->afx_hdl->usd_pub_tx_cookie,
		...
		const wl_event_msg_t *e, void *data)
{
	...
	if (event_type == WLC_E_ACTION_FRAME_COMPLETE) {
		WL_INFORM(("WLC_E_ACTION_FRAME_COMPLETE, RX packetID %0x,"
			"datalen %d\n", packetID, datalen));
	} else if (event_type == WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE) {
		WL_INFORM(("WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE, RX packetID %0x,"
			"datalen %d\n", packetID, datalen));
	}
	if (wl_get_drv_status(cfg, USD_PUB_ACTIVE, ndev)) {
		wl_handle_usd_tx_complete(cfg, cfgdev, e, packetID);
	}
...
wl_cfgp2p_tx_action_frame(struct bcm_cfg80211 *cfg, bcm_struct_cfgdev *cfgdev,
	...
	struct mgmt_tx_ctx *mgmt_tx_ctx)
{
	s32 ret = BCME_OK;
	s32 evt_ret = BCME_OK;
	s32 dwell_time = 0;
	...
	if (ret < 0) {
		...
		goto exit;
	} else if (mgmt_tx_ctx->flags & WL_ACT_FRAME_FLAG_NAN_PUB_USD) {
		wl_set_drv_status(cfg, USD_PUB_ACTIVE, dev);
		goto exit_usd;
	}
...
exit:
	...
	if (!(mgmt_tx_ctx->flags & WL_ACT_FRAME_FLAG_NAN_PUB_USD)) {
		...
		evt_ret = wl_cfg80211_apply_eventbuffer(bcmcfg_to_prmry_ndev(cfg), cfg, &buf);
		if (evt_ret < 0) {
