...
#define MAX_NUM_PEERS 1
typedef struct wl_pmsr_info {
	struct cfg80211_pmsr_request *pmsr_req;
	struct delayed_work pmsr_timeout_work;
	u16 session_id[MAX_NUM_PEERS];
	bool pmsr_complete;
} wl_pmsr_info_t;
#endif /* WL_PMSR */
...
#ifdef WL_PMSR
	struct wireless_dev *pd_wdev;	/* representing cfg cfg80211 device for PD */
	struct mutex pmsr_sync; /* pmsr sync for different contexts */
	u8 pd_state;
#endif /* WL_PMSR */
