				 ...
				 struct edgetpu_event_register __user *argp)
{
	...
	if (perdie_event_id_to_num(eventreg.event_id) >= EDGETPU_NUM_PERDIE_EVENTS)
		return -EINVAL;

	return gcip_event_mgr_set(etdev->event_mgr, perdie_event_id_to_num(eventreg.event_id),
					      ...
					      uint event_id)
{
	...
	gcip_event_mgr_unset(etdev->event_mgr, perdie_event_id_to_num(event_id));
