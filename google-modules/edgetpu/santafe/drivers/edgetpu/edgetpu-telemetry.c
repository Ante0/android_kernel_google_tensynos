...
int edgetpu_telemetry_init(struct edgetpu_dev *etdev)
{
	...
	for (i = 0; i < etdev->num_telemetry_buffers; i++) {
		ret = gcip_telemetry_init(
			&etdev->telemetry_log[i], GCIP_TELEMETRY_TYPE_LOG, etdev->dev,
			etdev->event_mgr,
			perdie_event_id_to_num(EDGETPU_PERDIE_EVENT_LOGS_AVAILABLE));
		if (ret)
			break;

		ret = gcip_telemetry_init(
			&etdev->telemetry_trace[i], GCIP_TELEMETRY_TYPE_TRACE, etdev->dev,
			etdev->event_mgr,
			perdie_event_id_to_num(EDGETPU_PERDIE_EVENT_TRACES_AVAILABLE));
		if (ret) {
