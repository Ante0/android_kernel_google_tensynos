 ...
 * @stats: Statistics of netdevice's tx/rx packets count and total length.
 ...
 * @intf_id: The netdevice's id, such as wwan0,wwan1,...
...
int mtk_wwan_recv(struct mtk_data_blk *data_blk, struct sk_buff *skb)
{
	...
	id = pkt_info->rx.id;
	wwan_inst = data_blk->wcb->wwan_inst[pkt_info->rx.ch_id];
...
static netdev_tx_t mtk_wwan_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	pkt_info->tx.q_id = skb_get_queue_mapping(skb);
...
static u16 mtk_wwan_select_queue(struct net_device *dev, struct sk_buff *skb,
				 struct net_device *sb_dev)
{
	...
	data_blk = wwan_inst->wcb->data_blk;
			 ...
			 enum mtk_data_cmd_type cmd, void *data)
{
	...
	if (ret < 0) {
		...
		MTK_ERR(wwan_inst->wcb->mdev,
			"Failed to execute command:ret=%d,cmd=%d\n", ret, cmd);
	}
...
static int mtk_wwan_start_txq(struct mtk_wwan_ctlb *wcb, u32 qmask)
{
	...
	for (i = 0; i < wcb->wwan_inst_num; i++) {
		...
		for (qno = 0; qno < wcb->data_blk->trans_info.txq_cnt; qno++) {
			if (qmask & BIT(qno)) {
...
static int mtk_wwan_netdev_init(struct mtk_wwan_ctlb *wcb)
{
	...
	for (i = 0; i < wcb->wwan_inst_num; i++) {
		dev = alloc_netdev_mqs(sizeof(*wwan_inst), "wwan%d", NET_NAME_UNKNOWN,
				       mtk_wwan_setup, wcb->data_blk->trans_info.txq_cnt,
				       wcb->data_blk->trans_info.rxq_cnt);
		if (unlikely(!dev)) {
			/* note that */
		...
		if (ret) {
			MTK_ERR(wcb->mdev, "Failed to register netdev, ret=%d\n", ret);
			free_netdev(dev);
			goto err;
		}
