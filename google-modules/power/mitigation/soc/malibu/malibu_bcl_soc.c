...
#define OCP_BATFET_TIMER_MAXIMUM_MS	3000
				...
				uint8_t config, uint16_t value, uint32_t *response)
{
	...
	if (response) {
		response[MB_BCL_IDX_RSP_STATUS] = resp_msg.payload[0];
		response[MB_BCL_IDX_RSP_DATA] = resp_msg.payload[1];
	}
...
{
	enum SUBSYSTEM_SOURCE i;
	int ret;
	uint16_t packed_clk_div_ratios = 0;
	uint32_t response[2] = {0, 0};

	for (i = 0; i < SOC_SUBSYSTEM_SOURCE_MAX; i++) {
		...
		if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS]) {
		...
		/* TODO: Support Dynamic Clock Division for AUR, TPU, GPU, DSU */
		if (i != CPU1A && i != CPU1B && i != CPU2)
			continue;

		/* Dynamic clock division enable/disable for the domain */
		ret = google_bcl_cpm_send_cmd(
			bcl_dev, MB_BCL_CMD_SET_CONFIG, i,
			MB_BCL_CONFIG_DYN_CLK_DIV_ENABLE,
			bcl_dev->subsys_dyn_clk_conf[i].enable, response);
		if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
			dev_err(bcl_dev->device,
				"Cannot set dynamic clock division enable %d\n",
				i);

		/* Pack clock division ratios for all oplvl tiers */
		packed_clk_div_ratios =
			(bcl_dev->subsys_dyn_clk_conf[i].ratio[OPLVL_HIGH] &
			 CLK_RATIO_MASK) |
			((bcl_dev->subsys_dyn_clk_conf[i].ratio[OPLVL_MID_HIGH] &
			  CLK_RATIO_MASK)
			 << (1 * CLK_RATIO_SHIFT)) |
			((bcl_dev->subsys_dyn_clk_conf[i].ratio[OPLVL_MID_LOW] &
			  CLK_RATIO_MASK)
			 << (2 * CLK_RATIO_SHIFT)) |
			((bcl_dev->subsys_dyn_clk_conf[i].ratio[OPLVL_LOW] &
			  CLK_RATIO_MASK)
			 << (3 * CLK_RATIO_SHIFT));
		ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, i,
					      MB_BCL_CONFIG_DYN_CLK_DIV_RATIO,
					      packed_clk_div_ratios, response);
		if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS]) {
			dev_err(bcl_dev->device,
				"Cannot set dynamic clock division ratio %d\n",
				i);
			continue;
		}
		...
		ret = google_bcl_cpm_send_cmd(
			bcl_dev, MB_BCL_CMD_SET_CONFIG, i,
			MB_BCL_CONFIG_DYN_CLK_DIV_HIGH_THRESH,
			bcl_dev->subsys_dyn_clk_conf[i].thresh[OPLVL_HIGH],
			response);

		if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS]) {
			dev_err(bcl_dev->device,
				"Cannot set dynamic clock division high oplv threshold %d\n",
				i);
			continue;
		}
		...
		ret = google_bcl_cpm_send_cmd(
			bcl_dev, MB_BCL_CMD_SET_CONFIG, i,
			MB_BCL_CONFIG_DYN_CLK_DIV_MID_HIGH_THRESH,
			bcl_dev->subsys_dyn_clk_conf[i].thresh[OPLVL_MID_HIGH],
			response);
		if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS]) {
			dev_err(bcl_dev->device,
				"Cannot set dynamic clock division medium oplv threshold %d\n",
				i);
			continue;
		}
		...
		ret = google_bcl_cpm_send_cmd(
			bcl_dev, MB_BCL_CMD_SET_CONFIG, i,
			MB_BCL_CONFIG_DYN_CLK_DIV_MID_LOW_THRESH,
			bcl_dev->subsys_dyn_clk_conf[i].thresh[OPLVL_MID_LOW],
			response);
		if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS]) {
			dev_err(bcl_dev->device,
				"Cannot set dynamic clock division low oplv threshold %d\n",
				i);
			continue;
		}
		...
		ret = google_bcl_cpm_send_cmd(
			bcl_dev, MB_BCL_CMD_SET_CONFIG, i,
			MB_BCL_CONFIG_DYN_CLK_DIV_LOW_THRESH,
			bcl_dev->subsys_dyn_clk_conf[i].thresh[OPLVL_LOW],
			response);
		if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS]) {
			dev_err(bcl_dev->device,
				"Cannot set dynamic clock division no need to apply the DFS OPLV threshold. %d\n",
				i);
			continue;
		}
	}
					    ...
					    bool hw_enabled)
{
	...
	for (i = 0; i <= SUBSYSTEM_SOURCE_MAX && i < AUR; i++) {
...
void google_bcl_set_bat_throttle_timer(struct bcl_device *bcl_dev)
{
	int ret;
	uint32_t response[2] = {0, 0};

	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, 0,
				      MB_BCL_CONFIG_OCP_BAT_THROTTLE_TIMER_SET, 1, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device, "Cannot cancel OCP THROTTLE timer\n");
}
...
void google_bcl_cancel_bat_throttle_timer(struct bcl_device *bcl_dev)
{
	int ret;
	uint32_t response[2] = {0, 0};

	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, 0,
				      MB_BCL_CONFIG_OCP_BAT_THROTTLE_TIMER_CANCEL, 1, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device, "Cannot cancel OCP BAT THROTTLE timer\n");
...
static void google_bcl_setup_batfet_timer(struct bcl_device *bcl_dev)
{
	...
	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, 0,
				      MB_BCL_CONFIG_OCP_BAT_THROTTLE_TIMEOUT,
				      bcl_dev->ocp_bat_throttle_timeout, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device, "Cannot set OCP BAT THROTTLE timeout\n");

	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, 0,
				      MB_BCL_CONFIG_OCP_BAT_THROTTLE_TIMEOUT_ENABLE,
				      bcl_dev->ocp_bat_throttle_timeout_enable, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device, "Cannot set OCP BAT THROTTLE timeout enable\n");
}
...
void google_bcl_setup_irq_mb(struct bcl_device *bcl_dev)
{
	google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_INIT, 0, 0, 0, NULL);
}
...
void google_bcl_parse_clk_div_dtree(struct bcl_device *bcl_dev)
{
	...
	bcl_dev->core_conf[DSU].con_heavy = get_prop_u32(np, "dsu_con_heavy");
	bcl_dev->core_conf[DSU].con_light = get_prop_u32(np, "dsu_con_light");
	bcl_dev->core_conf[CME].con_heavy = get_prop_u32(np, "cme_con_heavy");
	bcl_dev->core_conf[CME].con_light = get_prop_u32(np, "cme_con_light");
	...
	bcl_dev->core_conf[DSU].clkdivstep = get_prop_u32(np, "dsu_mitigation_res_en");
	bcl_dev->core_conf[CME].clkdivstep = get_prop_u32(np, "cme_mitigation_res_en");
	...
	bcl_dev->core_conf[DSU].mitigation_type = get_prop_u32(np, "dsu_mitigation_type");
	bcl_dev->core_conf[CME].mitigation_type = get_prop_u32(np, "cme_mitigation_type");
	...
	/* TODO rename cpu0_cluster/cpu1_cluster to cpu1a_cluster, cpu1b_cluster */
	...
	bcl_dev->subsys_dyn_clk_conf[CPU2].enable =
		of_property_read_bool(np, "cpu2_dyn_clk_div_en");
	bcl_dev->subsys_dyn_clk_conf[CPU1A].enable =
		of_property_read_bool(np, "cpu1a_dyn_clk_div_en");
	bcl_dev->subsys_dyn_clk_conf[CPU1B].enable =
		of_property_read_bool(np, "cpu1b_dyn_clk_div_en");
	of_property_read_u8_array(np, "cpu2_dyn_clk_div_ratio",
				  bcl_dev->subsys_dyn_clk_conf[CPU2].ratio,
				  OPLVL_TIER_COUNT);
	of_property_read_u8_array(np, "cpu1a_dyn_clk_div_ratio",
				  bcl_dev->subsys_dyn_clk_conf[CPU1A].ratio,
				  OPLVL_TIER_COUNT);
	of_property_read_u8_array(np, "cpu1b_dyn_clk_div_ratio",
				  bcl_dev->subsys_dyn_clk_conf[CPU1B].ratio,
				  OPLVL_TIER_COUNT);
	of_property_read_u32_array(np, "cpu2_dyn_clk_oplv_thresh",
				   bcl_dev->subsys_dyn_clk_conf[CPU2].thresh,
				   OPLVL_TIER_COUNT);
	of_property_read_u32_array(np, "cpu1a_dyn_clk_oplv_thresh",
				   bcl_dev->subsys_dyn_clk_conf[CPU1A].thresh,
				   OPLVL_TIER_COUNT);
	of_property_read_u32_array(np, "cpu1b_dyn_clk_oplv_thresh",
				   bcl_dev->subsys_dyn_clk_conf[CPU1B].thresh,
				   OPLVL_TIER_COUNT);
}
...
ssize_t get_ocp_bat_throttle_timeout_enable(struct bcl_device *bcl_dev, char *buf)
{
	uint32_t response[] = {0, 0};

	google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_GET_CONFIG, 0,
				MB_BCL_CONFIG_OCP_BAT_THROTTLE_TIMEOUT_ENABLE, 0, response);
	return sysfs_emit(buf, "%d\n", response[MB_BCL_IDX_RSP_DATA]);
}

ssize_t set_ocp_bat_throttle_timeout_enable(struct bcl_device *bcl_dev,
					    const char *buf, size_t size)
{
	...
	if (ret)
		return -EINVAL;
	if (bcl_dev->ocp_bat_throttle_timeout_enable == value)
		return size;

	bcl_dev->ocp_bat_throttle_timeout_enable = value;
	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, 0,
				      MB_BCL_CONFIG_OCP_BAT_THROTTLE_TIMEOUT_ENABLE,
				      (bool)value, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
...
ssize_t get_dyn_clk_div_enable(struct bcl_device *bcl_dev, int idx, char *buf)
{
	return sysfs_emit(buf, "%d\n",
			  bcl_dev->subsys_dyn_clk_conf[idx].enable);
}

ssize_t set_dyn_clk_div_enable(struct bcl_device *bcl_dev, const char *buf,
			       size_t size, int idx)
{
	bool value;
	int ret;
	...
	bcl_dev->subsys_dyn_clk_conf[idx].enable = value;
	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, idx,
				      MB_BCL_CONFIG_DYN_CLK_DIV_ENABLE,
				      (bool)value, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
...
ssize_t get_dyn_clk_div_ratio(struct bcl_device *bcl_dev, int idx, char *buf)
{
	return sysfs_emit(buf, "%u,%u,%u,%u\n",
			  bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_HIGH],
			  bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_MID_HIGH],
			  bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_MID_LOW],
			  bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_LOW]);
}

ssize_t set_dyn_clk_div_ratio(struct bcl_device *bcl_dev, const char *buf,
			      size_t size, int idx)
{
	unsigned int high_ratio, mid_high_ratio, mid_low_ratio, low_ratio;
	uint16_t tmp = 0;
	int ret;
	uint32_t response[2] = {0, 0};

	if (sscanf(buf, "%u,%u,%u,%u", &high_ratio, &mid_high_ratio,
		   &mid_low_ratio, &low_ratio) != 4)
		return -EINVAL;
	if (!bcl_dev)
		return -EIO;
	if (high_ratio > MAX_DYN_CLK_DIV_RATIO ||
	    mid_high_ratio > MAX_DYN_CLK_DIV_RATIO ||
	    mid_low_ratio > MAX_DYN_CLK_DIV_RATIO ||
	    low_ratio > MAX_DYN_CLK_DIV_RATIO)
		return -EINVAL;

	bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_HIGH] = high_ratio;
	bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_MID_HIGH] = mid_high_ratio;
	bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_MID_LOW] = mid_low_ratio;
	bcl_dev->subsys_dyn_clk_conf[idx].ratio[OPLVL_LOW] = low_ratio;

	tmp |= (high_ratio & 0x7);
	tmp |= (mid_high_ratio & 0x7) << 3;
	tmp |= (mid_low_ratio & 0x7) << 6;
	tmp |= (low_ratio & 0x7) << 9;

	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, idx,
				      MB_BCL_CONFIG_DYN_CLK_DIV_RATIO, tmp,
				      response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device,
			"Cannot set dynamic clock division ratio %d\n", idx);
...
ssize_t get_dyn_clk_div_thresh(struct bcl_device *bcl_dev, int idx, char *buf)
{
	return sysfs_emit(buf, "%u,%u,%u,%u\n",
			  bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_HIGH],
			  bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_MID_HIGH],
			  bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_MID_LOW],
			  bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_LOW]);
}

ssize_t set_dyn_clk_div_thresh(struct bcl_device *bcl_dev, const char *buf,
			       size_t size, int idx)
{
	uint32_t high_thresh, mid_high_thresh, mid_low_thresh, low_thresh;
	int ret;
	uint32_t response[2] = {0, 0};

	if (sscanf(buf, "%u,%u,%u,%u", &high_thresh, &mid_high_thresh, &mid_low_thresh,
		   &low_thresh) != 4)
		return -EINVAL;
	if (!bcl_dev)
		return -EIO;

	bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_HIGH] = high_thresh;
	bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_MID_HIGH] = mid_high_thresh;
	bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_MID_LOW] = mid_low_thresh;
	bcl_dev->subsys_dyn_clk_conf[idx].thresh[OPLVL_LOW] = low_thresh;

	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, idx,
				      MB_BCL_CONFIG_DYN_CLK_DIV_HIGH_THRESH,
				      high_thresh, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device,
			"Cannot set dynamic clock division high thresh %d\n",
			idx);

	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, idx,
				      MB_BCL_CONFIG_DYN_CLK_DIV_MID_HIGH_THRESH,
				      mid_high_thresh, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device,
			"Cannot set dynamic clock division mid thresh %d\n",
			idx);
	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, idx,
				      MB_BCL_CONFIG_DYN_CLK_DIV_MID_LOW_THRESH,
				      mid_low_thresh, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device,
			"Cannot set dynamic clock division low thresh %d\n",
			idx);
	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, idx,
				      MB_BCL_CONFIG_DYN_CLK_DIV_LOW_THRESH,
				      low_thresh, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device,
			"Cannot set dynamic clock division none thresh %d\n",
			idx);
	return size;
}

ssize_t get_ocp_bat_throttle_timeout(struct bcl_device *bcl_dev, char *buf)
{
	uint32_t response[] = {0, 0};

	/* Mailbox payload only supports sending value of size 16bits */
	google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_GET_CONFIG, 0,
				MB_BCL_CONFIG_OCP_BAT_THROTTLE_TIMEOUT, 0, response);
...
ssize_t set_ocp_bat_throttle_timeout(struct bcl_device *bcl_dev, const char *buf, size_t size)
{
	uint32_t value;
	int ret;
	uint32_t response[2] = {0, 0};
	...
	if (value > OCP_BATFET_TIMER_MAXIMUM_MS || value < OCP_BATFET_TIMER_MINIMUM_MS)
		return -EINVAL;
	if (bcl_dev->ocp_bat_throttle_timeout == value)
		return size;

	bcl_dev->ocp_bat_throttle_timeout = value;
...
int google_init_vd_soc(struct bcl_device *bcl_dev)
{
	int ret;
	uint32_t response[2] = {0, 0};

	ret = google_bcl_cpm_send_cmd(bcl_dev, MB_BCL_CMD_SET_CONFIG, 0,
				      MB_BCL_CONFIG_VDROOP_DMUX_SET, 1, response);
	if (ret < 0 || response[MB_BCL_IDX_RSP_STATUS])
		dev_err(bcl_dev->device, "Cannot set VDROOP DMUX");
