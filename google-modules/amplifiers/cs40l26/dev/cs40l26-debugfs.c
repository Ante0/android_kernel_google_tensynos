		...
		struct cs40l26_wseq_params *wseq_params, char **wseq_str)
{
	...
	*wseq_str = kzalloc(str_size, GFP_KERNEL);
	...
	while (!cl_dsp_memchunk_end(&ch)) {
		...
		error = scnprintf(str, CS40L26_WSEQ_STR_LINE_LEN,
		...
		loff_t *ppos)
{
	...
	error = scnprintf(result, count, "0x%08X\n", cs40l26->dbg_hw_reg);
	if (error <= 0) {
		error = -EINVAL;
		goto err_mutex;
	}
		...
		loff_t *ppos)
{
	...
	error = scnprintf(result, CS40L26_HW_STR_LEN, "0x%08X\n", val);
	if (error <= 0) {
		error = -EINVAL;
		goto err_mutex;
	}
...
void cs40l26_debugfs_init(struct cs40l26_private *cs40l26)
{
	...
	error = scnprintf(name, CS40L26_DEBUGFS_DIR_NAME_LEN, "%s-%d-00%X",
