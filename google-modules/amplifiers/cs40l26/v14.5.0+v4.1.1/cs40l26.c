		...
		struct cs40l26_wseq_params *wseq_params, size_t nbytes, u32 *reg_addr)
{
	...
	size_t bytes_read = 0;
	u32 current_read_addr = wseq_params->base_addr;

	if (nbytes > wseq_params->max_size_bytes) {
		dev_warn(cs40l26->dev, "Requested read %zu > max %zu, limiting\n",
			nbytes, wseq_params->max_size_bytes);
		nbytes = wseq_params->max_size_bytes;
	}
	...
	while (bytes_read < nbytes) {
		size_t bytes_to_read = min(nbytes - bytes_read, (size_t) CS40L26_WSEQ_STR_LINE_LEN);

		error = regmap_raw_read(cs40l26->regmap, current_read_addr, seq_data + bytes_read,
					bytes_to_read);
		if (error) {
			dev_err(cs40l26->dev, "Failed regmap_raw_read chunk from 0x%08X (size %zu): %d\n",
					current_read_addr, bytes_to_read, error);
			goto err_free;
		}

		bytes_read += bytes_to_read;
		current_read_addr += bytes_to_read;
	}
	if (error)
		goto err_free;
