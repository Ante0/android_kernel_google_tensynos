...
static int aw_check_register_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	DBG("bin_num=%d, parse_register_num = 0x%x, check_register_num = 0x%x\n",
...
static int aw_check_dsp_reg_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	DBG("bin_num=%d, parse_dsp_reg_num = 0x%x, check_dsp_reg_num = 0x%x\n",
...
static int aw_check_soc_app_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	DBG("bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = 0x%x\n",
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((aw_dev->i2c->adapter->nr == cfg_dde[i].dev_bus) &&
			...
			if (cfg_dde[i].data_type != ACF_SEC_TYPE_MONITOR) {
				ret = aw_dev_parse_data_by_sec_type(aw_dev,
					prof_hdr, &cfg_dde[i],
...
static int aw_dev_parse_get_valid_prof(struct aw_device *aw_dev,
		...
		struct aw_cfg_hdr *prof_hdr)
{
	...
	ret = aw_dev_parse_get_valid_prof(aw_dev, all_prof_info);
	if (ret < 0)
		return ret;
...
static int aw_dev_parse_drv_type_v_1_0_0_0(struct aw_device *aw_dev,
	struct aw_cfg_hdr *prof_hdr, struct aw_cfg_dde_v_1_0_0_0 *cfg_dde,
	int *cur_scene_id)
{
	int ret = -1;
	struct aw_prof_info *prof_info = &aw_dev->prof_info;
		...
		if ((is_default &&
			...
			for (j = 0; j < valid_ddt_count; j++) {
				...
				for (k = 0; k < valid_ddt_count - 1 - j; k++)
					...
					valid_ddt_table[valid_ddt_count - k - 2];
				valid_ddt_table[j] = i;
				break;
			}
		...
		struct aw_cfg_hdr *prof_hdr)
{
	...
	for (i = 0; i < prof_hdr->a_ddt_num; i++) {
		if ((cfg_dde[i].type == AW_SKT_TYPE_ID) &&
			...
			(aw_dev->channel == cfg_dde[i].dev_index) &&
			(aw_dev->chip_id == cfg_dde[i].chip_id)) {
			ret = aw_dev_parse_get_scene_id_v_1_0_0_0(aw_dev,
				cfg_dde[i].dev_profile_str, &scene_id);
			if (ret < 0) {
	...
	struct aw_container *aw_cfg)
{
	...
	ret = aw_dev_parse_get_scene_count_v1_0_0_0(aw_dev, aw_cfg,
		&prof_info->count, &is_default);
	if (ret < 0) {
