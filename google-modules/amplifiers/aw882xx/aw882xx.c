...
#define AW882XX_DRIVER_VERSION "dv2.0.5"
...
static struct aw_component_codec_ops aw_component_codec_ops = {
	...
	struct snd_soc_dai *dai)
{
	...
	struct aw882xx *aw882xx =
		aw_component_codec_ops.codec_get_drvdata(codec);
			 ...
			 struct snd_ctl_elem_info *uinfo)
{
	...
	aw_snd_soc_codec_t *codec =
		aw_component_codec_ops.kcontrol_codec(kcontrol);
	struct aw882xx *aw882xx =
		aw_component_codec_ops.codec_get_drvdata(codec);
...
{
	int ret = 0;
	aw_snd_soc_codec_t *codec = aw_component_codec_ops.kcontrol_codec(kcontrol);
	struct aw882xx *aw882xx = aw_component_codec_ops.codec_get_drvdata(codec);
	...
	/* size might be sizeof(struct algo_auth_data) + 2 * sizeof(unsigned int) for tinymix */
	if (size < sizeof(struct algo_auth_data)) {
		aw_dev_err(aw882xx->dev, "size[%d] < algo_auth_data size[%lu]",
				size, sizeof(struct algo_auth_data));
		return -EINVAL;
	}
...
{
	struct snd_kcontrol_new *aw882xx_dev_control;
	char *kctl_name;

	aw882xx_dev_control = devm_kcalloc(aw882xx->codec->dev, AW_KCTL_NUM,
				sizeof(struct snd_kcontrol_new), GFP_KERNEL);
	if (aw882xx_dev_control == NULL)
		return -ENOMEM;

	kctl_name = devm_kasprintf(aw882xx->codec->dev, GFP_KERNEL,
		"aw_dev_%d_prof", aw882xx->aw_pa->channel);
	if (!kctl_name)
		return -ENOMEM;
	...
	kctl_name = devm_kasprintf(aw882xx->codec->dev, GFP_KERNEL,
		"aw_dev_%d_monitor", aw882xx->aw_pa->channel);
	if (!kctl_name)
		return -ENOMEM;
	...
	kctl_name = devm_kasprintf(aw882xx->codec->dev, GFP_KERNEL,
		"aw_dev_%d_volume", aw882xx->aw_pa->channel);
	if (!kctl_name)
		return -ENOMEM;
	...
	kctl_name = devm_kasprintf(aw882xx->codec->dev, GFP_KERNEL,
		"aw_dev_%d_hal_mon_work", aw882xx->aw_pa->channel);
	if (!kctl_name)
		return -ENOMEM;
	...
	kctl_name = devm_kasprintf(aw882xx->codec->dev, GFP_KERNEL,
		"aw_dev_%d_offset", aw882xx->aw_pa->channel);
	if (!kctl_name)
		return -ENOMEM;
	...
	kctl_name = devm_kasprintf(aw882xx->codec->dev, GFP_KERNEL,
		"aw_dev_%d_iv_output", aw882xx->aw_pa->channel);
	if (!kctl_name)
		return -ENOMEM;
	...
	aw_component_codec_ops.add_codec_controls(aw882xx->codec,
...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	/*
	 * b/419610004
	 * Move outside aw882xx->lock scope to prevent ABBA deadlock detected by debug build.
	 * aw882xx_dynamic_create_controls() is used to create tinymix controls that are used
	 * by user space process (tinymix or tinyalsa). Since there is already locked to protect
	 * by sound system, we don't need additional lock to protect it here.
	 *
...
static void aw882xx_add_codec_controls(struct aw882xx *aw882xx)
{
	...
	aw_component_codec_ops.add_codec_controls(aw882xx->codec, aw882xx_dev_control, 1);
...
{
	struct aw882xx *aw882xx =
		aw_component_codec_ops.codec_get_drvdata(aw_codec);
	aw_dev_info(aw882xx->dev, "enter");
...
static int aw_component_codec_register(struct aw882xx *aw882xx)
{
	int ret = 0;
...
static int aw_disabled_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	aw_pr_info("aw hac amp is not probed\n");
	return 0;
}

static const struct snd_kcontrol_new aw_controls_nop[] = {
	SOC_SINGLE_EXT("HAC AMP Disabled", SND_SOC_NOPM, 0, 0, 0,
		aw_disabled_get, NULL),
};

static const struct snd_soc_component_driver soc_codec_dev_aw_nop = {
	.controls = aw_controls_nop,
	.num_controls = ARRAY_SIZE(aw_controls_nop),
};

static int aw_pcm_startup_nop(struct snd_pcm_substream *substream,
			       struct snd_soc_dai *dai)
{
	aw_pr_info("%s: enter dai->name = %s\n", __func__, dai->name);
	return 0;
}

static const struct snd_soc_dai_ops aw_ops_nop = {
	.startup = aw_pcm_startup_nop,
};

static int aw_component_nop_codec_register(struct aw882xx *aw882xx)
{
	int ret = 0;
	struct snd_soc_dai_driver *dai_drv = NULL;
	...
	memcpy(dai_drv, aw882xx_dai, sizeof(aw882xx_dai));
	dai_drv->ops = &aw_ops_nop;

	ret = aw882xx->codec_ops->register_codec(aw882xx->dev,
			&soc_codec_dev_aw_nop,
			dai_drv, ARRAY_SIZE(aw882xx_dai));
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "failed to register nop codec: %d", ret);
		return -EINVAL;
	}
...
static struct aw882xx *aw882xx_malloc_init(struct i2c_client *i2c)
{
	...
	aw882xx->i2c = i2c;
	aw882xx->codec = NULL;
	aw882xx->codec_ops = &aw_component_codec_ops;
	aw882xx->fw_status = AW_DEV_FW_FAILED;
	aw882xx->fw_retry_cnt = 0;
...
int aw882xx_hw_reset(struct aw882xx *aw882xx)
{
	...
	if (gpio_is_valid(aw882xx->reset_gpio)) {
		aw_dev_info(aw882xx->dev, "set value 0");
		gpio_set_value_cansleep(aw882xx->reset_gpio, 0);
		mdelay(10);
		aw_dev_info(aw882xx->dev, "set value 1");
...
#endif
{
	...
	if (ret) {
		aw_dev_err(&i2c->dev, "codec init failed");
		goto reg_nop_codec;
	}
	...
	/*codec register*/
	ret = aw_component_codec_register(aw882xx);
	if (ret) {
...
err_sysfs:
	aw_component_codec_ops.unregister_codec(&i2c->dev);

reg_nop_codec:
	ret = aw_component_nop_codec_register(aw882xx);
	if (ret)
		aw_dev_err(&i2c->dev, "nop codec register failed");
	else
		aw_dev_info(&i2c->dev, "nop codec register success");
	return ret;
}
