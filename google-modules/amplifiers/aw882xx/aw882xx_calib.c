...
static bool is_single_cali; /*if multi_dev cali false, single dev true*/
...
static int aw_cali_svc_dev_cali_mode_en(struct aw_device *aw_dev,
	int type, bool is_enable, unsigned int flag)
{
	int ret;
...
int aw_cali_svc_get_devs_st(struct aw_device *aw_dev, int32_t *st_buf, int num)
{
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel < num) {
			ret = aw882xx_dsp_read_st(local_dev,
				&st_buf[local_dev->channel << 1],
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (is_single_cali) {
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
				len += snprintf(buf+len, PAGE_SIZE-len, "%s:%d mOhms ",
					ch_name[i], cali_re[i]);
			len += snprintf(buf+len, PAGE_SIZE-len, "\n");
		}
	}
...
static int aw_cali_misc_open(struct inode *inode, struct file *file)
{
	...
	if (local_dev->channel != g_dev_select) {
		...
		aw_dev_dbg(local_dev->dev,
			"can not find dev num %d, use default", g_dev_select);
	}
			...
			unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case AW_IOCTL_SET_PTR_PARAM_NUM: {
			ret = aw_cali_misc_params_ptr(aw_dev,
				(struct ptr_params_data *)data_ptr);
		} break;
			...
			unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case AW_IOCTL_GET_F0_Q: {
			data_32_ptr = (int32_t *)data_ptr;
			ret = aw882xx_dsp_read_f0_q(aw_dev,
				&data_32_ptr[0], &data_32_ptr[1]);
		} break;
...
static ssize_t aw_cali_misc_read(struct file *filp,
...
static int aw_cali_misc_switch_dev(struct file *filp,
	struct aw_device *aw_dev, char *cmd_buf)
{
	int ret;
	int i;
