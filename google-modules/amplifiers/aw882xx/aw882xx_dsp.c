...
#ifdef AW_MTK_PLATFORM_WITH_DSP
	ret = mtk_spk_recv_ipi_buf_from_dsp((char *)dsp_msg,
		sizeof(aw_msg_hdr_t) + len, &real_len);
#else
...
int aw882xx_dsp_read_dsp_msg(struct aw_device *aw_dev,
...
int aw882xx_dsp_write_dsp_msg(struct aw_device *aw_dev,
		uint32_t msg_id, char *data_ptr, unsigned int size)
{
	int ret = 0;
...
int aw882xx_dsp_read_f0_q(struct aw_device *aw_dev, int32_t *f0, int32_t *q)
{
	...
	ret = aw882xx_dsp_read_dsp_msg(aw_dev, AW_MSG_ID_F0_Q,
		(char *)data, sizeof(int32_t) * 4);
	if (ret) {
...
int aw882xx_dsp_read_vol_offset(struct aw_device *aw_dev, int32_t *voltage_offset)
{
	int ret = 0;
...
int aw882xx_dsp_write_vol_offset(struct aw_device *aw_dev, int32_t voltage_offset)
{
	int ret = 0;
	uint32_t msg_id = 0;
