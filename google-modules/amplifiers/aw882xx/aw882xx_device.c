...
void aw_dev_algo_authentication(struct aw_device *aw_dev)
{
	...
	ret = aw882xx_dsp_read_algo_auth_data(aw_dev,
		(char *)&algo_data, sizeof(struct algo_auth_data));
	if (ret < 0)
		goto exit;
	...
	ret = aw882xx_dsp_write_algo_auth_data(aw_dev,
		(char *)&algo_data, sizeof(struct algo_auth_data));
	if (ret < 0)
		goto exit;
	...
	if (reg_addr == aw_dev->volume_desc.reg) {
		if (aw_dev->ops.aw_reg_val_to_db) {
			aw_dev->volume_desc.init_volume =
				aw_dev->ops.aw_reg_val_to_db((reg_val &
...
int aw882xx_dev_init_cali_re(struct aw_device *aw_dev)
{
	...
	if (cali_desc->mode) {
		...
		aw_dev_dbg(aw_dev->dev, "no cali, needn't init cali re");
	}
	return ret;
}
...
static void aw_dev_parse_fade_flag_dt(struct aw_device *aw_dev)
{
	int ret;
	uint32_t fade_en = 0;
