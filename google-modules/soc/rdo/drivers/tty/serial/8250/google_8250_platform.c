...
void google_serial8250_set_isa_configurator(serial8250_isa_config_fn v)
{
...
static void __init __serial8250_isa_init_ports(void)
{
	...
	for (i = 0; i < nr_uarts; i++)
		google_serial8250_setup_port(i);
	...
	for (i = 0; i < ARRAY_SIZE(old_serial_port) && i < nr_uarts; i++) {
		struct uart_8250_port *up = google_serial8250_get_port(i);
...
static int serial8250_probe_acpi(struct platform_device *pdev)
{
	...
	line = google_serial8250_register_8250_port(&uart);
	if (line < 0)
		return line;
...
static int __init serial8250_init(void)
{
	...
	ret = google_univ8250_console_init();
	if (ret == 0)
		goto out;
...
static void __exit serial8250_exit(void)
{
	...
	/*
	 * This tells google_serial8250_unregister_port() not to re-register
