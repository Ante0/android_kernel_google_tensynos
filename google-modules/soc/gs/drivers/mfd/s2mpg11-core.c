...
static struct i2c_client *s2mpg11_get_i2c_client(struct s2mpg11_dev *dev,
						 unsigned int reg)
{
...
static int s2mpg11_regmap_read_reg(void *context, unsigned int reg,
				   unsigned int *dest)
{
	u8 ureg = reg;
	u8 *udest = (u8 *)dest;
...
static int s2mpg11_regmap_write_reg(void *context, unsigned int reg,
				    unsigned int value)
{
	u8 ureg = reg;
	u8 uvalue = value;
