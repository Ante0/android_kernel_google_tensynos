...
int32_t NepModemBufferPoolSetup(void)
{
	...
	ret = RingServiceRegisterIsr(NOA_PORT_MODEM_FW, kNoaModemNepBufferPool,
...
#define D2H_CACHED_RING_ITEM_LEN (NOA_DESC_MODEM_LASSEN_BYTE)
				  ...
				  void *vpn_desc)
{
	...
	struct noa_rx_ipsec_metadata *metadata =
		&((struct mr_lassen_ext_rxd *)dst->ext_data)->ipsec_metadata;
	memcpy(dst, src, noa_desc_bytes(src->desc_type));
	dst->reason = FWD_REASON_VPN;
	dst->desc_type = NOA_DESC_MODEM_LASSEN;
	...
	memcpy(metadata, &entry->ipsec_metadata, sizeof(struct noa_rx_ipsec_metadata));
}
#endif /* linux */
...
int32_t NepModemDeviceToHostPathSetup(void)
{
	...
	task = NepTaskGet(kNepCacheDataTaskGroup, kNepCacheModemDeviceToHostPathTask);
	if (!task) {
		pr_err("Failed to get modem device to host cache stage task\n");
		return -EINVAL;
	}
	cache_stage_params.name = "modem_d2h_cache";
	...
	if (ret) {
		pr_err("Failed to setup cache stage on modem device to host path, ret %" PRId32
		       "\n",
		       ret);
		return -EINVAL;
	}
	...
	task = NepTaskGet(kNepFetchHeaderTaskGroup, kNepFetchHeaderModemDeviceToHostPathTask);
	if (!task) {
		pr_err("Failed to get modem fetch header stage task\n");
		return -EINVAL;
	}
	fetch_header_stage_params.name = "modem_header";
	...
	if (ret) {
		pr_err("Failed to setup fetch header stage on modem path, ret %" PRId32 "\n", ret);
		return -EINVAL;
	}
	...
	task = NepTaskGet(kNepAnalyzeDataTaskGroup, kNepAnalyzeModemPacketTask);
	if (!task) {
		pr_err("Failed to get modem tethering stage task\n");
		return -EINVAL;
	}
	tethering_stage_params.name = "modem_tethering";
	...
	if (ret) {
		pr_err("Failed to setup tethering stage on modem path, ret %" PRId32 "\n", ret);
		return -EINVAL;
	}
	...
	task = NepTaskGet(kNepAnalyzeDataTaskGroup, kNepVpnModemRxPacketTask);
	if (!task) {
		pr_err("Failed to get modem VPN RX stage task\n");
		return -EINVAL;
	}
	vpn_stage_params.name = "modem_vpn_rx";
	vpn_stage_params.is_wlan = false;
	...
	if (ret) {
		pr_err("Failed to setup VPN RX stage on modem path, ret %" PRId32 "\n", ret);
		return -EINVAL;
	}
	...
	/****************************** Route Data Stage ******************************/
	NestedCachedRingInit(&cached_ring, &cached_ring_buf[0], D2H_CACHED_RING_ITEM_LEN,
			     D2H_CACHED_RING_SIZE, &cached_ring_vpn_stage_tail, &cached_ring_tail);

	NestedShadowRingInit(&shadow_ring, (uintptr_t)&shadow_ring_buf[0], D2H_SHADOW_RING_ITEM_LEN,
			     D2H_CACHED_RING_SIZE, &shadow_ring_vpn_stage_tail, &shadow_ring_tail);

	task = NepTaskGet(kNepRouteDataTaskGroup, kNepRouteModemDeviceToHostPathTask);
	if (!task) {
		pr_err("Failed to get Modem device to host route stage task\n");
		return -EINVAL;
	}
	route_stage_params.name = "modem_d2h_route";
	...
	route_stage_params.feedthrough_path_notifier =
		RingServiceDoorbellNotifierGet(NOA_PORT_MODEM_SW);
	route_stage_params.feedback_path_notifier =
		RingServiceDoorbellNotifierGet(NOA_PORT_MODEM_FW);
	route_stage_params.forward_wlan_path_notifier =
		RingServiceDoorbellNotifierGet(NOA_PORT_WLAN_FW);
#ifdef linux
	...
	if (ret) {
		pr_err("Failed to setup route stage on modem device to host path, ret %" PRId32
		       "\n",
		       ret);
		return -EINVAL;
	}
	...
	ret = RingServiceRegisterIsr(NOA_PORT_MODEM_FW, kNoaModemRingRxData, cache_stage.task);
	if (ret) {
		pr_err("Failed to register ISR for modem device to host path, ret %" PRId32 "\n",
		       ret);
		return -EINVAL;
	}
...
#define H2D_CACHED_RING_ITEM_LEN (NOA_DESC_MODEM_LASSEN_BYTE)
#define H2D_SHADOW_RING_ITEM_LEN (32U)
#define H2D_SHADOW_RING_BYTES (H2D_SHADOW_RING_ITEM_LEN * H2D_CACHED_RING_SIZE)

static void MovePacketToNewBuffer(void *d, void *entry)
{
	int32_t ret;
	...
	if (desc->desc_type != NOA_DESC_MODEM_LASSEN) {
		NepFillHostEntry(desc, entry);
		return;
	}

	// TODO - This buffer pool 'get' logic may cause a race condition, but it is
	// currently used only for Lassen's Gem5 testing.
	ret = NepBufferPoolGetBuffer(context, &buffer_desc);
	if (ret) {
	...
	((nep_host_entry *)entry)->ipsec_metadata =
		((struct mr_lassen_ext_txd *)desc->ext_data)->ipsec_metadata;
#endif /* linux */
	NepFillHostEntry(desc, entry);

	noa_ring_tail_write_once(&context->instance->ring,
				 NepBufferPoolSrcRingConsumerIdxGet(context));
	NepBufferPoolCompleted(context, NepBufferPoolCachedRingConsumerIdxGet(context));
}
...
int32_t NepModemHostToDevicePathSetup(void)
{
	...
	cache_stage_params.cache_context = &cache_context;
	cache_stage_params.path_id = src_path_id;
	cache_stage_params.fill_entry = MovePacketToNewBuffer;
#ifdef linux
	cache_stage_params.dma = NULL;
#else /* linux */
	...
	task = NepTaskGet(kNepAnalyzeDataTaskGroup, kNepVpnModemTxPacketTask);
	if (!task) {
		pr_err("Failed to get modem VPN TX stage task\n");
		return -EINVAL;
	}
	vpn_stage_params.name = "modem_vpn_tx";
	vpn_stage_params.is_wlan = false;
	...
	ret = NestedRingVpnStageSetup(&vpn_stage_params);
	if (ret) {
		pr_err("Failed to setup VPN TX stage on modem path, ret %" PRId32 "\n", ret);
		return -EINVAL;
	}
	NepRingStageRegister(src_ring, &vpn_stage);
	...
	ret = RingServiceRegisterIsr(NOA_PORT_MODEM_SW, kNoaModemRingTxData, cache_stage.task);
	if (ret) {
