...
#include <wlan/dhd_custom_google_noa.h>
#include <wlan/dhd_custom_google_noa_trace.h>
#include <common/ring_id.h>
#include <common/wlan_ring_id.h>

#include "noa_wlan_buffer_management.h"
#if IS_ENABLED(CONFIG_NOA_VPN_OFFLOAD_SUPPORT)
#include <dhd_linux_priv.h> // For dhd_info_t
...
static int dhd_noa_dump_bus(void *priv, char *buf, int len)
{
	...
	cnt += scnprintf(
		buf + cnt, len - cnt,
		"pcie: %s, irq: %d,intr: %d, irq_registered: %d, intr_enabled: %d, multi-en: %d\n",
		pci_name(bus->dev), bus->dev->irq, bus->intr, bus->irq_registered,
		bus->intr_enabled, MULTIBP_ENAB(bus->sih));
	cnt += scnprintf(buf + cnt, len - cnt,
...
static int dhd_noa_dump_rings(void *priv, char *buf, int len)
{
	...
	cnt += scnprintf(buf + cnt, len - cnt,
			 ...
			 prot->max_rxbufpost, prot->rxbufpost, prot->rx_bufpost_threshold);
...
{
	dhdpcie_bus_intr_disable((dhd_bus_t *)priv);
...
static void dhd_client_ring_query(dhd_bus_t *bus, msgbuf_ring_t *ring,
				  struct noa_wlan_ring_info *info, bool d2h)
{
	u32 addr;
	u8 *va;
	dhd_pub_t *dhd = bus->dhd;
...
{
	u16 read = readl((void *)info->regs.read);
	u16 write = readl((void *)info->regs.write);
...
static int dhd_noa_wlan_bus_init(struct noa_wlan_client *client)
{
	...
	dhd_client_ring_query(bus, &prot->d2hring_rx_cpln, &client->rx_ring[0], true);
	/* TX CPL ring info query*/
	client->tx_cpl_flow_max = 1;
	dhd_client_ring_query(bus, &prot->d2hring_tx_cpln, &client->tx_cpl_ring[0], true);
	/* TX ring info*/
	client->tx_flow_max = dhd_get_max_flow_rings(dhd);
	if (prot->h2d_flowrings_pool) {
		for (i = 0; i < bus->max_tx_flowrings; i++) {
			dhd_client_ring_query(bus, &prot->h2d_flowrings_pool[i],
					      &client->tx_ring[i], false);
		}
	}
	/* RX post info */
	client->rx_post_max = 1;
	dhd_client_ring_query(bus, &prot->h2dring_rxp_subn, &client->rx_post_ring[0], false);
	return 0;
}
...
static int dhd_noa_wlan_start(void *priv)
{
	...
	client->doorbell_addr = PCI_16KB0_PCIREGS_OFFSET;
	client->doorbell_addr +=
		(DAR_ACTIVE(bus->dhd)) ? DAR_PCIH2D_DB0_0(bus->sih->buscorerev) : PCIH2D_MailBox;
	/* nep information */
...
int _dhd_noa_wlan_rxbm_sync(void *priv, u32 num, void **rxbm, bool to_dev)
{
	...
	bufs = kzalloc(sizeof(struct noa_bm_buf) * RXBM_BUSRT_SIZE, GFP_KERNEL);
	if (!bufs)
		return total;
...
static void bcm_txd_feedback_handle(struct noa_wlan_client *client, struct noa_desc *d)
{
	struct noa_bcm_txd *bcm_txd = (struct noa_bcm_txd *)d->ext_data;
	host_txbuf_cmpl_t txstatus = {
		.compl_hdr = {
			.flow_ring_id = bcm_txd->ring_id + 2,
			.status = 0,
		},
		.cmn_hdr = {
			.request_id = d->tkid,
	...
	dhd_noa_txcpl_sync(client->bus, &txstatus);
}

static int feedback_event_handle(struct noa_wlan_client *client, struct noa_desc *d)
{
	if (d->reason != FWD_REASON_VPN)
		return 0;

	switch (d->desc_type) {
	case NOA_DESC_WLAN_TX_BRCM:
		bcm_txd_feedback_handle(client, d);
		break;
	default:
		noa_wlan_err("%s(): error feedback desc type %d\n", __func__, d->desc_type);
		break;
	}
	return 0;
}

extern bool dhd_prot_process_msgbuf_rxcpl_packet(dhd_pub_t *dhd, void *rxd);
static int dhd_noa_wlan_rx_data(struct noa_wlan_client *client, struct noa_desc *d)
{
	dhd_bus_t *bus = client->bus;
	dhd_pub_t *dhd = bus->dhd;
	dhd_prot_t *prot = dhd->prot;
...
static int dhd_noa_wlan_rx(struct noa_wlan_client *client, void **msg)
{
	...
	case NOAD_MODE_FEEDBACK:
		return feedback_event_handle(client, d);
	case NOAD_MODE_DATA:
		return dhd_noa_wlan_rx_data(client, d);
	default:
		return -EINVAL;
	}
}
...
static int _dhd_noa_wlan_tx(struct noa_wlan_client *client, void *pkt,
			    host_txbuf_post_v1_t *tx_desc)
{
	dhd_bus_t *bus = client->bus;
	dhd_pub_t *dhd = bus->dhd;
	...
	txd->ring_id = DHD_PKT_GET_FLOWID(pkt) - 2;
	...
	memcpy(d->ext_data, txd, sizeof(struct noa_bcm_txd));
	return 0;
}
...
{
	dhd_bus_t *bus = (dhd_bus_t *)client->bus;
...
#endif
		/* Disable the IRQ to ensure safe freeing the IRQ */
		dhdpcie_disable_irq(bus);
		dhdpcie_free_irq(bus);
		bus->intr = false;
	}
	/* PCIe irq is registered by NCP WiFi firmware */
	if (!IS_ENABLED(CONFIG_NOA_SIM_SUPPORT)) {
		pdev->irq = client->hw.irq;
		noa_wlan_err("%s(): replace irq from %d to %d\n", __func__, client->irqs[0],
			     client->hw.irq);
		if (request_irq(client->hw.irq, dhdpcie_isr, IRQF_SHARED, pci_name(pdev), bus) < 0)
			return -ENOMEM;
	}
...
static int dhd_noa_wlan_txq_active(void *priv, u16 flowid, bool enable)
{
	...
	return noa_wlan_fw_txq_active(client, flowid - 2, enable);
}
