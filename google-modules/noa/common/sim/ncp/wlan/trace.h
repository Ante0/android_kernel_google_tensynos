 ...
 */
#undef TRACE_SYSTEM
#define TRACE_SYSTEM ncp_noa_wlan
...
/* NOA packet capture */
DECLARE_EVENT_CLASS(wlan_noa_pkt_cap_class,
	...
	TP_printk("flow_id: %u, pkt_len: %u, pkt_buff: %s, data_len: %u, data_buff: %s",
		...
		__print_array(__get_dynamic_array(pkt_buff),
				__get_dynamic_array_len(pkt_buff) / sizeof(char),
				sizeof(char)),
		__entry->data_len,
		__print_array(__get_dynamic_array(data_buff),
				__get_dynamic_array_len(data_buff) / sizeof(char),
				sizeof(char)))
);
#define DEFINE_NOA_PKT_CAP_EVENT(name)	\
DEFINE_EVENT(wlan_noa_pkt_cap_class, name,	\
	TP_PROTO(u32 flow_id, u32 pkt_len, const char *pkt, u32 data_len, const char *data), \
	TP_ARGS(flow_id, pkt_len, pkt, data_len, data))
DEFINE_NOA_PKT_CAP_EVENT(wlan_noa_dump_vendor_tx);
DEFINE_NOA_PKT_CAP_EVENT(wlan_noa_dump_vendor_rx);
