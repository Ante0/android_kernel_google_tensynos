...
static int modem_fw_input_ring_plat_activate(int ring_type)
{
	if (md_plat_ops.md_fw_input_ring_activate) {
		return md_plat_ops.md_fw_input_ring_activate(
...
static int modem_fw_input_ring_plat_deactivate(int ring_type)
{
	if (md_plat_ops.md_fw_input_ring_deactivate) {
		return md_plat_ops.md_fw_input_ring_deactivate(
			NoaRingPathIdConvert(
				kNoaNetworkInterfaceModem,
				kNoaNetworkFlowDeviceToHost,
				ring_type));
...
static int modem_fw_output_ring_plat_activate(int ring_type)
{
	if (md_plat_ops.md_fw_output_ring_activate) {
		return md_plat_ops.md_fw_output_ring_activate(
			NoaRingPathIdConvert(
				kNoaNetworkInterfaceModem,
...
static int modem_fw_output_ring_plat_deactivate(int ring_type)
{
	if (md_plat_ops.md_fw_output_ring_deactivate) {
		return md_plat_ops.md_fw_output_ring_deactivate(
			NoaRingPathIdConvert(
				kNoaNetworkInterfaceModem,
				kNoaNetworkFlowHostToDevice,
				ring_type));
...
static int modem_fw_ring_activate(struct noa_ring_wrapper *ring, int ring_type)
{
	...
	if (noa_ring_is_consumer(ring)) {
		...
		nep_ring_service_request_send(
			...
			kNoaRingNepOutput);
#else
		if (modem_fw_output_ring_plat_activate(ring_type)) {
			NCP_MD_TX_ERROR("Failed to do activate modem fw output ring");
			return -ENOEXEC;
		}
#endif
	} else {
		...
		nep_ring_service_request_send(
			...
			kNoaRingNepInput);
#else
		if (modem_fw_input_ring_plat_activate(ring_type)) {
			NCP_MD_RX_ERROR("Failed to do activate modem fw input ring");
			return -ENOEXEC;
		}
#endif
	}
...
static int modem_fw_ring_deactivate(struct noa_ring_wrapper *ring, int ring_type)
{
	if (noa_ring_is_consumer(ring)) {
		...
		nep_ring_service_request_send(
			...
			kNoaRingNepOutput);
#else
		if (modem_fw_output_ring_plat_deactivate(ring_type)) {
				NCP_MD_TX_ERROR("Failed to do deactivate modem fw output ring");
				return -ENOEXEC;
		}
#endif
	} else {
		...
		nep_ring_service_request_send(
			...
			kNoaRingNepInput);
#else
		if (modem_fw_input_ring_plat_deactivate(ring_type)) {
				NCP_MD_RX_ERROR("Failed to do deactivate modem fw input ring");
				return -ENOEXEC;
		}
#endif
	}
...
#else
		SEC_EXRAM_DATA static char rx_buffer[
			NOA_DESC_MODEM_RX_MTK_MSG_PD_BYTE * NOA_FW_RING_SIZE] = { 0 };
		buf = (void *)rx_buffer;
#endif
...
static int modem_fw_tx_ring_init(struct noa_md_fw *md_fw)
{
	...
	buf = modem_fw_ring_dma_alloc(md_fw, tx_ring, NOA_DESC_MODEM_TX_MTK_BYTE *
								     NOA_FW_RING_SIZE);
#else
	SEC_EXRAM_DATA static char tx_buffer[
		NOA_DESC_MODEM_TX_MTK_BYTE * NOA_FW_RING_SIZE] = { 0 };
	buf = (void *)tx_buffer;
#endif
...
#ifndef linux
int modem_fw_ring_plat_init(struct modem_fw_ring_plat_params *params)
{
	memset((void *)&md_plat_ops, 0, sizeof(md_plat_ops));
	md_plat_ops.md_fw_input_ring_activate = params->md_fw_input_ring_activate;
	md_plat_ops.md_fw_input_ring_deactivate = params->md_fw_input_ring_deactivate;
	md_plat_ops.md_fw_output_ring_activate = params->md_fw_output_ring_activate;
	md_plat_ops.md_fw_output_ring_deactivate = params->md_fw_output_ring_deactivate;
	return 0;
}
#endif
...
#else
int modem_fw_nep_interrupt_handler(int32_t id, void *data)
#endif
{
	NCP_MD_TX_INFO("enter");
	(void)id;
...
#else
	ret = modem_fw_register_nep_mailbox(md_fw);
#endif
	return ret;
}
