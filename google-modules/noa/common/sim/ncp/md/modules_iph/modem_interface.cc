...
{
	NCP_MD_INFO("enter, type=[%d], q_mask=[%d]", type, q_mask);
	switch (type) {
...
#else
int noa_dpmaif_event_handler(noa_dpmaif_drv_intr_type event_type, unsigned int queue_mask)
{
	switch (event_type) {
		case NOA_DPMAIF_INTR_UL_DONE:
			noa_ncp_md_tx_irq_tx_done(g_md_fw, queue_mask);
			break;
		...
		case NOA_DPMAIF_INTR_DL_DONE:
			noa_ncp_md_rx_dpmaif_event_handle(g_md_fw, event_type, queue_mask);
			break;
		case NOA_DPMAIF_INTR_TRAS_SYNC:
			// TODO: Call dpmaif_irq_tras_sync();
			break;
		default:
			break;
	}
