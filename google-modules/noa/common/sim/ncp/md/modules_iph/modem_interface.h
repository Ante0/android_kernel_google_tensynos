...
#else
int noa_dpmaif_event_handler(
		noa_dpmaif_drv_intr_type event_type, unsigned int queue_mask);

#define NOA_MD_SEND_DOORBELL(ring_type, queue_id, count) \
    noa_dpmaif_drv_send_doorbell((noa_dpmaif_drv_ring_type)ring_type, queue_id, count)
#define NOA_MD_INTERRUPT_COMPLETE(intr_type, queue_id, data) \
    noa_dpmaif_drv_intr_complete((noa_dpmaif_drv_intr_type)intr_type, queue_id, data)
#define NOA_MD_GET_RING_INDEX(ring_index, queue_id) \
    noa_dpmaif_drv_get_ring_idx((noa_dpmaif_ring_index)ring_index, queue_id)
#endif
