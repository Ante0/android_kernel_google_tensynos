	...
	unsigned int *pkt_cnt)
{
	...
	uint32_t bat_idx, rx_tkid, mapped_rx_tkid;
	...
	int ret;
	struct noa_modem_rx_vendor_msg_pd_desc raw_desc = {0};
	struct noa_modem_rx_vendor_msg_pd_desc *p_desc_msg_pd = &raw_desc;
	struct noa_modem_rx_vendor_pd_desc raw_pd_desc = {0};
	...
	unsigned long long data_dma_addr_high, data_dma_addr_low;
	unsigned int data_len, pit_hd_offset;
	unsigned int noa_ring_cnt = 0;
	unsigned int rx_cnt;
	bool is_noa_ring_begin_processing = false;
	...
	for (rx_cnt = 0; rx_cnt < pit_cnt; rx_cnt++) {
		...
		if (noa_ring_cnt == 0) {
			...
			if (ret <= 0) {
				NCP_MD_RX_ERROR("Fail to noa_ring_begin_processing ret=[%d]", ret);
				return ret;
			}
			is_noa_ring_begin_processing = true;
		}
...
out:
	if (noa_ring_cnt > 0) {
		noa_ring_complete_processing(ring);
		noa_ring_cnt = 0;
	} else if (is_noa_ring_begin_processing) {
		noa_ring_complete_processing(ring);
		is_noa_ring_begin_processing = false;
	} else {
		NCP_MD_RX_ERROR("Do Nothing");
	}
...
static int noa_ncp_md_rx_data_collect_more(struct noa_rx_queue *rxq, int *work_done)
{
...
void noa_ncp_md_rx_dpmaif_event_handle(
	...
	case DPMAIF_INTR_DL_DONE:
		// NOA md data rx step 11: According to the interrupt type,
...
static void noa_ncp_md_rx_reload_bat(struct noa_md_fw_rx *rx, struct noa_bat_ring *bat_ring)
{
	...
	unsigned short rx_tkid, mapped_rx_tkid;
	...
	while (rx_tkid_info->rx_tkid_free_rear != rx_tkid_info->rx_tkid_free_fore) {
		...
		memcpy(bat_ring->bat_base + bat_idx,
		...
		if (bat_ring->type == NORMAL_BAT) {
			...
			bat_ring->noa_data_addr[mapped_rx_tkid].noa_va =
				rx_tkid_info->free_pool[rx_tkid_info->rx_tkid_free_fore].noa_data_addr;
		} else { /* FRAG_BAT */
			mapped_rx_tkid =
				noa_ncp_md_bat_rx_tkid_map(bat_ring->id, bat_ring->type, rx_tkid);
			bat_ring->noa_data_addr[mapped_rx_tkid].noa_va =
	...
	if (doorbell_cnt > 0) {
		...
		if (bat_ring->type == NORMAL_BAT) {
			...
			if (unlikely(ret < 0)) {
				NCP_MD_RX_ERROR("Fail to send doorbell, ncp FRAG BAT(%d)", bat_ring->id);
			}
#if NCP_DEBUG
			NCP_MD_RX_INFO("ring_type=[%d], ring_id[%d], bat_wr_idx(csr)=[%d]",
				       bat_ring->type, bat_ring->id,
	...
	u64 noa_data_addr)
{
	...
	rx_tkid_info->rx_tkid_free_rear =
			noa_ncp_md_rx_ring_next_idx(bat_ring->bat_cnt, rx_tkid_info->rx_tkid_free_rear);
...
int noa_ncp_md_rx_init(struct noa_md_fw *md_fw)
{
	...
	NCP_MD_RX_INFO("rxq_cnt:%u", md_fw->rx->rxq_cnt);
