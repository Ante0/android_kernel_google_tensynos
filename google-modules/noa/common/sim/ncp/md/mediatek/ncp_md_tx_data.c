...
static noa_buffer_pool_desc g_pool_ring_buf[NOA_MD_FW_FIFO_SIZE] = { 0 };
...
{
	NCP_MD_DATA_LIMIT(
		"enter, md_fw=%p, q_mask=%d", md_fw, q_mask);

	struct noa_md_fw_tx *tx = md_fw->tx;

	if (!tx) {
		NCP_MD_TX_ERROR_LIMIT("tx is null");
		return;
	}
	...
	for (int i = 0; i < tx->txq_cnt; i++) {
		...
		if (ulq_done) {
			...
			int drb_rd_idx;
			NCP_MD_DATA_LIMIT("ulq_done for queue_id=%u", i);
			...
			NCP_MD_DATA_LIMIT("drb_rd_idx:%u", txq->drb_rd_idx);
			schedule_delayed_work(&txq->tx_done_work, msecs_to_jiffies(0));
		}
	}
	NCP_MD_DATA_LIMIT("exit");
}
