...
#include <wcn7760/linux/btpower.h>
...
static const char *const tcs_seq_str[BTPOWER_TCS_SEQ_MAX] = {"upval", "dwnval", "enable"};
...
static struct bt_power bt_vreg_info_wcn7760 = {
	.compatible = "qcom,wcn7760",
	.vregs = NULL,
	.num_vregs = 0,
};

static const struct of_device_id bt_power_match_table[] = {
	{	.compatible = "qcom,wcn7760", .data = &bt_vreg_info_wcn7760},
	{},
};
...
static int bt_configure_gpios(struct btpower_platform_data *drvdata, bool on)
{
	...
	int bt_reset_control_gpio = drvdata->bt_gpio_sys_control;

	if (!on)
		bt_configure_wakeup_gpios(drvdata, on);
	/* always reset the controller no matter ON or OFF */
	if (gpio_is_valid(bt_reset_control_gpio))
		gpio_set_value_cansleep(bt_reset_control_gpio, 1);
	...
	LOGI(drvdata, "BT-ON asserting BT_EN");
				 ...
				 struct bt_power_vreg_data *vreg)
{
	...
	LOGD(drvdata,
		"Got regulator: %s, min_vol: %u, max_vol: %u, load_curr: %u, is_retention_supp: %u",
...
static int btpower_gpios_source_initialize(struct btpower_platform_data *drvdata)
{
	...
	if (gpio_is_valid(drvdata->bt_gpio_sys_control)) {
		rc = btpower_gpio_acquire_output(drvdata, drvdata->bt_gpio_sys_control,
						"bt_sys_control", 1);
		if (rc) {
			drvdata->bt_gpio_sys_control = -1;
			goto gpio_failure;
		}
	}
...
static void btpower_gpios_source_release(struct btpower_platform_data *drvdata)
{
	...
	if (gpio_is_valid(drvdata->bt_gpio_sys_control)) {
		gpio_free(drvdata->bt_gpio_sys_control);
		drvdata->bt_gpio_sys_control = -1;
	}
}
				      ...
				      struct btpower_platform_data *drvdata)
{
	...
	drvdata->bt_gpio_sys_control =
		of_get_named_gpio(pdev->dev.of_node, "qcom,bt-reset-ctrl-gpio", 0);
	if (!gpio_is_valid(drvdata->bt_gpio_sys_control)) {
		LOGE(drvdata, "bt-reset-control not provided in device tree");
		return -EIO;
	}
...
static long btpower_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case BT_FMD_OP: {
		bool bt_fmd_enabled_ = (bool)arg;

		LOGW(drvdata, "BT_FMD_OP value=%s", bt_fmd_enabled_ ? "True" : "False");
		drvdata->bt_fmd_enabled = bt_fmd_enabled_;
		wcn_set_fmd_state(bt_fmd_enabled_);
		break;
	}
