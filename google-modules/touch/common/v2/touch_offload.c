	...
	for (i = 0; i < frame->num_channels; i++) {
		if (frame->channel_type[i] == TOUCH_DATA_TYPE_COORD)
			...
			channel_size = TOUCH_OFFLOAD_FRAME_SIZE_STYLUS_STATUS;
				...
				CONTEXT_CHANNEL_TYPE_VENDOR_REGISTER)
			channel_size = TOUCH_OFFLOAD_FRAME_SIZE_VENDOR_REGISTER;
		else {
	...
	for (i = 0; i < nb; i++) {
		...
		for (mask = CONTEXT_CHANNEL_BIT_START;
			...
			case CONTEXT_CHANNEL_TYPE_STYLUS_STATUS:
				...
				break;
			case CONTEXT_CHANNEL_TYPE_VENDOR_REGISTER:
				size =
				    TOUCH_OFFLOAD_FRAME_SIZE_VENDOR_REGISTER;
				break;
			default:
