...
#define to_sw_emulator(emulator) container_of(emulator, struct software_emulator, base)

static enum hrtimer_restart software_emulator_timer_callback(struct hrtimer *timer)
{
...
static int software_emulator_interruptible_usleep(struct software_emulator *sw_emul, ktime_t ktime)
{
...
static int software_emulator_process_frames(struct software_emulator *sw_emul)
{
	...
	ktime_t target_time = ktime_add(sw_emul->start_timestamp,
					ktime_sub(frame_time, sw_emul->first_frame_timestamp));

	if (ktime_to_ns(sw_emul->previous_frame_timestamp) >= ktime_to_ns(target_time)) {
	...
	if (ktime_to_ns(sleep_time) > 0) {
		...
		ret = software_emulator_interruptible_usleep(sw_emul, sleep_time);
		if (ret) {
...
static int software_emulator_thread_func(void *sw_emul_self)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ret == -EBUSY) {
			software_emulator_interruptible_usleep(sw_emul, ms_to_ktime(100));
			continue;
		} else if (ret) {
			pr_warn("touch_sim_peak_and_load_frame error, ret %u", ret);
...
static int software_emulator_start(struct emulator *emulator)
{
	...
	sw_emul->thread = kthread_run(software_emulator_thread_func, sw_emul, "touch_sim_thread");
...
static bool software_emulator_is_ready(struct emulator *emulator)
{
	...
	return !IS_ERR_OR_NULL(sw_emul->thread) && sw_emul->state == SW_EMULATOR_NORMAL;
};
