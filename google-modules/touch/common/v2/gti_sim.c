...
int touch_sim_on_frame_processed(struct touch_sim *sim, ktime_t timestamp)
{
	...
	return sim->pop_data_cb(sim->private_data, sim->temp_frame, sim->frame_size, timestamp);
}
...
int touch_sim_peak_and_load_frame(struct touch_sim *sim)
{
	...
	if (kfifo_out_peek(&sim->fifo, &frame_size, sizeof(frame_size)) != sizeof(frame_size)) {
		pr_warn("peek failed");
		return -EIO;
	}

	if (frame_size != sim->frame_size) {
		pr_err("frame size not match first frame_size(%u vs %u)", frame_size,
		       sim->frame_size);
		return -EIO;
	}

	if (frame_size > kfifo_len(&sim->fifo)) {
		pr_warn("size not enough frame_size(%u) kfifo_len:%u", frame_size,
			kfifo_len(&sim->fifo));
		return -EBUSY;
	}
...
static ssize_t touch_sim_add_frame_data(struct touch_sim *sim, struct file *file, u8 *buf,
					size_t count)
{
	...
	while (handle_count < count) {
		if (file->f_flags & O_NONBLOCK) {
			...
			if (wait_event_interruptible(sim->event_wait_queue,
...
static bool touch_sim_is_ready(struct touch_sim *sim)
{
	if (sim == NULL || !kfifo_initialized(&sim->fifo) || sim->temp_frame == NULL ||
	    sim->temp_frame_header == NULL || !emulator_has_function(sim->emulator, is_ready) ||
...
static int touch_sim_start(struct touch_sim *sim, u32 frame_size)
{
	...
	sim->temp_frame_header = kzalloc(sizeof(struct TouchOffloadFrameHeader), GFP_KERNEL);
...
static ssize_t touch_sim_read(struct file *file, char __user *user_buf, size_t count,
			      loff_t *offset)
{
	...
	long ret = copy_to_user(user_buf, &reported_frame_count, sizeof(reported_frame_count));

	if (ret != 0) {
		pr_err("copy_to_user(,%zd) failed, ret %ld", sizeof(reported_frame_count), ret);
		return -EFAULT;
	}
...
static int touch_sim_open(struct inode *inode, struct file *file)
{
	...
	if (sim == NULL || sim->pop_data_cb == NULL || sim->private_data == NULL) {
...
static int touch_sim_release(struct inode *inode, struct file *file)
{
	...
	pr_info("Frame count %llu, Average diff: %lld us, Max diff: %lld us, Min diff: %lld us",
		frame_count,
		frame_count > 0 ? (s64)(sim->total_diff_ns / (s64)frame_count) / NSEC_PER_USEC : 0,
		sim->max_diff_ns / NSEC_PER_USEC, sim->min_diff_ns / NSEC_PER_USEC);
...
static unsigned int touch_sim_poll(struct file *file, poll_table *wait)
{
	...
	if (kfifo_is_empty(&sim->fifo) && atomic_read(&sim->reported_frame_count) > 0)
