...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int read_sys_info(struct fts_ts_info *info)
{
	int res;
	...
	res = fts_get_checksum_enable(&info->checksum_enabled);
	if (res < OK) {
		pr_info("%s: ERROR %08X\n", __func__, res);
		return res;
	}
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
static int parse_bin_file(u8 *ubin_data, int ubin_size,
...
{
	int res = OK;
	u8 event_to_search = EVT_ID_NOEVENT;
	u8 *read_data = NULL;
...
int flash_update(struct fts_ts_info *info, struct force_update_flag *force_update)
{
	...
	info->checksum_force_disabled = true;

	res = flash_burn(info, fw, force_update);
	if (res < OK) {
		pr_err("%s: ERROR flash update %08X\n", __func__, res);
		info->checksum_force_disabled = false;
		goto goto_end;
	}
