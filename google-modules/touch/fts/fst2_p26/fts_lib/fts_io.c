...
static u8 calculate_checksum(u8 *data, int length);
...
static int fts_spi_sync(struct spi_message *msg)
{
	...
	ktime_t delta = ktime_sub(now, info->timestamp_last_spi_transfer);
	s64 delta_ns = ktime_to_ns(delta);
	s64 min_gap_ns = 5 * NSEC_PER_USEC;

	if (delta_ns < min_gap_ns) {
		delay_us = (min_gap_ns - delta_ns) / 1000;
		usleep_range(delay_us, delay_us + 10);
	}

	ret = gti_spi_sync(get_client(), msg);
	info->timestamp_last_spi_transfer = ktime_get();
	return ret;
}
...
/**
  ...
  * @return OK if success or an error code which specify the type of error
  */
int poll_for_event(u8 *event_to_search, int event_bytes, u8 *read_data,
						int read_length, int time_to_wait)
{
	...
	count_err = 0;
	pr_info("%s: %s\n", __func__,
		print_hex("SEARCH EVENT = ", event_to_search, event_bytes, temp, sizeof(temp)));
	msleep(TIMEOUT_RESOLUTION);
	while (find != 1 && retry < time_to_wait &&
			(fts_read_fw_fifo(read_data, read_length) >= OK)) {
		if (read_data[0] != EVT_ID_NOEVENT && !info->checksum_force_disabled &&
				info->checksum_enabled) {
			if (fts_validate_checksum(read_data, read_length) < 0) {
				retry++;
				msleep(TIMEOUT_RESOLUTION);
				continue;
			}
		...
		if (read_data[0] == EVT_ID_ERROR) {
			...
			case EVT_TYPE_ERROR_ITO_SENSEOPEN:
				...
				break;
			case EVT_TYPE_ERROR_VSYNC_STS_DEAD:
				printk("{VSync:Status Dead Error}\n");
				break;
			default:
				printk("\n");
				break;
...
{
	int res = 0;
	u8 event_to_search = EVT_ID_CONTROLLER_READY;
	u8 *read_data = NULL;
...
int fts_write_fw_reg(u16 address, u8 *data, uint32_t length)
{
	...
	if (length > SPI_REG_W_CHUNK)
		return ERROR_OP_NOT_ALLOW;

	if (!info->checksum_force_disabled && info->checksum_enabled) {
		if (address == FLASH_SAVE_ADDR || address == HDM_WRITE_REQ_ADDR ||
			address == HDM_REQ_ADDR || (address >= 0x0078 && address < 0x0080)) {
			checksum_length = 0;
		} else {
			checksum_length = 1;
		}
	...
	cmd_length = 3 + length + checksum_length;
	cmd = kmalloc(ALIGN(sizeof(u8) * cmd_length, 4), GFP_KERNEL);
	if (cmd == NULL) {
		pr_err("%s: Error allocating memory\n", __func__);
		return ERROR_BUS_W;
	}

	cmd[0] = FTS_CMD_REG_SPI_W;
	cmd[1] = (u8)((address >> 8) & 0xFF);
	cmd[2] = (u8)(address & 0xFF);
	memcpy(&cmd[3], data, length);
	if (checksum_length > 0)
		cmd[3 + length] = calculate_checksum(&cmd[1], 2 + length);

	pr_debug("%s: addr = %04X\n", __func__, address);
	for (i = 0; i < length; i++)
		pr_debug("%s: data[%d] = %02X\n", __func__, i, data[i]);

	res = fts_write(cmd, cmd_length);
	kfree(cmd);
#endif
	if (res < OK)
		pr_err("%s: ERROR %08X\n", __func__, res);
...
static int fts_validate_hdm_frame_data(struct frame_data *frame_data)
{
	u8 *data = NULL;
	u8 sum = 0;
	bool non_zero = false;
...
int fts_set_fw_settings(fw_settings_type_t type, u8 setting)
{
	...
	if (type == FW_SETTINGS_VSYNC_TEST) {
		return fts_set_fw_settings_with_poll_echo_event(type, setting);
	}
...
int fts_set_fw_settings_with_poll_echo_event(fw_settings_type_t type, u8 setting)
{
	int res = 0;
	u8 data[2] = { 0 };
	u8 event[2] = { FW_SETTINGS_ADDR & 0x00FF, type };
	struct fts_ts_info *info = dev_get_drvdata(get_dev());
	int retry = 0;
	...
	do {
		...
		retry++;
		res = fts_poll_status_echo_event(event, 2);
		if (res < OK) {
			pr_info("%s: Poll status echo event, type: %02X setting: %d. Retried: %d\n",
				__func__, type, setting, retry);
		}
	} while (retry < SPI_REG_W_RETRY && res < OK);

	if (retry == SPI_REG_W_RETRY && res < OK) {
		pr_err("%s: Failed to poll status echo event, type: %02X setting: %d. ERROR %08X\n",
			__func__, type, setting, res);
		res = ERROR_TIMEOUT;
	}

	if (res > OK) {
		pr_err("%s: found %d error events, type: %02X setting: %d. ERROR %08X\n",
			__func__, res, type, setting, ERROR_OP_NOT_ALLOW);
...
int fts_set_checksum_enable(bool enable)
{
	struct fts_ts_info *info = dev_get_drvdata(get_dev());
	int res = 0;
	u8 data = enable;

	res = fts_write_fw_reg(SPI_CHECKSUM_EN_ADDR, &data, 1);
	if (res < OK) {
		pr_err("%s: ERROR %08X\n", __func__, res);
		return res;
	}
	info->checksum_enabled = enable;
	return res;
}

int fts_get_checksum_enable(bool *enable)
{
	int res = 0;
	u8 data = 0;

	res = fts_read_fw_reg(SPI_CHECKSUM_EN_ADDR, &data, 1);
	if (res < OK) {
		pr_err("%s: ERROR %08X\n", __func__, res);
		return res;
	}
...
int fts_validate_checksum(u8 *data, int length)
{
	u8 expect_checksum = calculate_checksum(data, length - 1);
	u8 actual_checksum = data[length - 1];
	char temp[128] = { 0 };

	if (actual_checksum != expect_checksum) {
		pr_err("%s: %s", __func__, print_hex("data = ", data, length, temp, sizeof(temp)));
		pr_err("%s: invalid checksum, expect|actual = %02X|%02X. ERROR %08X\n",
			__func__, expect_checksum, actual_checksum, ERROR_CHECKSUM);
 ...
 * @return u8 Calculated checksum value.
 */
static u8 calculate_checksum(u8 *data, int length)
{
	u8 checksum = 0;
...
/**
  * Polling for status echo event
  * @return OK if success or an error code which specify the type of error
  */
int fts_poll_status_echo_event(u8 *data, uint32_t length)
{
	int res = 0;
	int event_to_search_length = 2 + length;
	u8 *event_to_search = NULL;
	u8 *read_data = NULL;

	if (fifo_evt_size < 2 + length) {
		pr_info("%s: invalid data length: %d ERROR %08X\n", __func__, length,
			ERROR_OP_NOT_ALLOW);
		return ERROR_OP_NOT_ALLOW;
	}

	event_to_search = (u8 *)kmalloc(event_to_search_length, GFP_KERNEL);
	if (event_to_search == NULL) {
		pr_info("%s: Error allocating memory\n", __func__);
		return ERROR_ALLOC;
	}
	...
	if (read_data == NULL) {
		pr_info("%s: Error allocating memory\n", __func__);
		kfree(event_to_search);
		event_to_search = NULL;
		return ERROR_ALLOC;
	}

	event_to_search[0] = EVT_ID_STATUS_UPDATE;
	event_to_search[1] = EVT_TYPE_STATUS_ECHO;
	memcpy(&event_to_search[2], data, length);

	res = poll_for_event(event_to_search, event_to_search_length,
		read_data, fifo_evt_size, TIMEOUT_FW_REG_STATUS);
	if (res < OK)
		pr_err("%s: ERROR %08X\n", __func__, res);
