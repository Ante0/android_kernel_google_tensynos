...
/**
  * Perform all the test related to Vsync signal
  * @return OK if success or an error code which specify the type of error
  */
int fts_production_test_vsync(void)
{
	int res = OK;
	int ret = OK;
	u8 a_event_to_search[2] = { EVT_ID_STATUS_UPDATE,
		EVT_TYPE_STATUS_VSYNC_TEST };
	u8 *read_data = NULL;
	...
	pr_info("%s: VSYNC SIGNAL TEST STARTING...\n", __func__);
	res = fts_set_fw_settings(FW_SETTINGS_VSYNC_TEST, true);
	if (res < OK) {
		pr_err("%s: ERROR %08X\n", __func__, res);
		res |= ERROR_PROD_TEST_VSYNC;
		ret |= res;
		goto goto_error;
	}

	res = poll_for_event(a_event_to_search, 2, read_data, fifo_evt_size,
		TIMEOUT_GENERAL);
	if (res == OK) {
		if (read_data[2] == 0x00 && read_data[3] == 0xFF) {
			pr_err("%s: ERROR Vsync is not enabled\n", __func__);
			res = (ERROR_PROD_TEST_VSYNC | ERROR_PROD_TEST_CHECK_FAIL);
			ret |= res;
		}
	} else {
		res |= ERROR_PROD_TEST_VSYNC;
...
goto_error:
	if (ret != OK) {
		pr_err("%s: VSYNC SIGNAL TEST:.................FAIL\n\n", __func__);
	} else {
		pr_info("%s: VSYNC SIGNAL TEST:.................OK\n", __func__);
	}
				...
				struct test_to_do *tests, int do_init)
{
	...
	pr_info("%s: [12]VSYNC TEST......\n",__func__);
	res = fts_production_test_flash_writing_enable(path_limits, tests);
	if (res != OK) {
		pr_info("%s: [12]VSYNC TEST FAIL\n",__func__);
		ret |= res;
		if (stop_on_fail)
			goto goto_error;
	}
