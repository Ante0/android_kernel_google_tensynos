...
#define FRAME_BUFFER_ADDR		0x0000 /* /< frame buffer
							 * address in memory */
#define SPI_REG_W_CHUNK			13 /* /< chunk dimension of a
...
#define SPI_REG_W_RETRY			3 /* /< number of retry in case of SPI
...
#define SCAN_MODE_ADDR			0x0010 /* /< FW reg scan mode
							 * address */
#define SPI_CHECKSUM_EN_ADDR		0x0016 /* /< FW reg spi checksum
							 * enable address */
...
#define EVT_TYPE_STATUS_VSYNC_TEST	0x23	/* /< VSync Test Status */
...
#else
#define spi_len_dma_align(len) ((len) >= 64) ? ALIGN(len, 16) : (len)
...
typedef enum {
	...
	FW_SETTINGS_WATER_MODE = 0x14, /* /< Water mode */
	...
	FW_SETTINGS_VSYNC_TEST = 0x60, /* /< Vsync test */
...
#pragma pack(1)
struct st_gui_debug_info {
	u16 frame_count;
	u8 scan_mode;
	u8 feature_status_param1;
	u8 feature_status_param2;
	u8 feature_status_param3;
	u8 reserve1[2];
	u8 feature_detect_param1;
	u8 feature_detect_param2;
	u8 feature_detect_param3;
	u8 feature_detect_param4;
	u16 feature_detect_param5;
	u16 feature_detect_param6;
	u8 feature_detect_param7;
	u8 feature_detect_param8;
	u8 feature_detect_param9;
	u8 reserve2[5];
	u8 noise_status_param1;
	u8 noise_status_param2;
	u8 noise_status_param3;
	u8 noise_status_param4;
	u16 noise_monitor_val[21];
	u8  reserve3[10];
	u16 fcal_monitor_param1;
	u16 fcal_monitor_param2;
	u8  fcal_monitor_param3;
	u8  fcal_monitor_param4;
	u8  fcal_monitor_param5;
	u8  res4[25];
	u16 feature_process_para[8];

	u8 reserve4[120]; // 0x80~0xF7 is reserved
};
...
struct frame_data_header {
	...
	struct st_gui_debug_info gui_debug_info; /* /< Debug info */
};
...
struct goog_debug_info {
	...
	u8 conti_rpt_en; /* /< Continuous report enable */
	u8 palm_rej_en; /* /< Palm reject enable */
	u8 grip_rej_en; /* /< Grip reject enable */
	u8 high_sens_mode_en; /* /< High sensitivity mode enable */
	u8 coor_fltr_en; /* /< Coordinate filter enable */
	u8 conti_rpt_state; /* /< Continuous report state */
	u8 palm_rej_state; /* /< Palm reject state */
	u8 disp_mode_det; /* /< Display mode detection */
	u8 external_mode_det; /* /< External mode detection */
	u8 radiation_mode_det; /* /< Radiation mode detection */
	u8 reserved2; /* /< Reserved */
	u16 ms_mrn_std_dev0; /* /< Mutual sense MRN standard deviation of
				 * frequency band R0 */
	u16 ms_mrn_std_dev1; /* /< Mutual sense MRN standard deviation of
				 * frequency band R1 */
	u16 ms_mrn_std_dev2; /* /< Mutual sense MRN standard deviation of
				 * frequency band R2 */
	u16 ms_mrn_std_dev3; /* /< Mutual sense MRN standard deviation of
				 * frequency band R3 */
	u16 ms_mrn_diff_std_dev0; /* /< Mutual sense MRN differential filter
				 * standard deviation of frequency band R0 */
	u16 ms_mrn_diff_std_dev1; /* /< Mutual sense MRN differential filter
				 * standard deviation of frequency band R1 */
	u16 ms_mrn_diff_std_dev2; /* /< Mutual sense MRN differential filter
				 * standard deviation of frequency band R2 */
	u16 ms_mrn_diff_std_dev3; /* /< Mutual sense MRN differential filter
				 * standard deviation of frequency band R3 */
	u8 reserved3[0x40]; /* /< Reserved */
};
...
char *print_hex(const char *label, const u8 *buff, int count, u8 *result,
		size_t result_sz);
int poll_for_event(u8 *event_to_search, int event_bytes,
