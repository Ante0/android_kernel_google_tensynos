...
#define CMD_VSYNCTEST			0xC2
/*!<  usage: echo C2 > /proc/fts/driver_test; cat /proc/fts/driver_test */
#define CMD_CHECKSUMENABLE			0xC3
/*!<  usage: echo C3 00/01 > /proc/fts/driver_test;
cat /proc/fts/driver_test ==> 00 to disable checksum, 01 to enable checksum */
#define CMD_SETFWSETTINGS			0xC4
/*!<  usage: echo C4 type 00/01 > /proc/fts/driver_test; cat /proc/fts/driver_test; */
#ifdef DEBUG
...
#ifdef DEBUG
int fts_set_fw_settings_mode_with_poll_echo_event(fw_settings_type_t type, uint8_t mode)
{
	int res = 0;
	u8 data[2] = { 0  };
	...
	do {
		...
		if (res < OK) {
			pr_info("%s: Poll status echo event, type: %02X mode: %d. Retried: %d\n",
				__func__, type, mode, retry);
		}
	} while (retry < SPI_REG_W_RETRY && res < OK);

	if (retry == SPI_REG_W_RETRY && res < OK) {
		pr_err("%s: Failed to poll status echo event, type: %02X mode: %d. ERROR %08X\n",
			__func__, type, mode, res);
		res = ERROR_TIMEOUT;
	}

	if (res > OK) {
		pr_err("%s: found %d error events, type: %02X mode: %d. ERROR %08X\n",
			__func__, res, type, mode, ERROR_OP_NOT_ALLOW);
...
int fts_set_fw_settings_mode(fw_settings_type_t type, uint8_t mode)
{
	...
	if (type == FW_SETTINGS_VSYNC_TEST) {
		return fts_set_fw_settings_mode_with_poll_echo_event(type, mode);
	}
			...
			size_t count, loff_t *pos)
{
	...
	if (number_param >= 1) {
		...
		case CMD_VSYNCTEST:
			if (number_param == 1) {
				res = fts_production_test_vsync();
				if (res < OK)
					pr_err("%s: Error running <VSYNC>tests: %08X\n",
						__func__, res);
			} else {
				pr_err("%s: wrong number of parameters\n", __func__);
				res = ERROR_OP_NOT_ALLOW;
			...
			break;
		case CMD_CHECKSUMENABLE:
			if (number_param == 2) {
				res = fts_set_checksum_enable(!!cmd[1]);
			} else {
				pr_err("%s: wrong number of parameters\n", __func__);
				res = ERROR_OP_NOT_ALLOW;
			}
			break;
		case CMD_SETFWSETTINGS:
			if (number_param == 3) {
				res = fts_set_fw_settings(cmd[1], !!cmd[2]);
			} else {
				pr_err("%s: wrong number of parameters\n", __func__);
				res = ERROR_OP_NOT_ALLOW;
			}
