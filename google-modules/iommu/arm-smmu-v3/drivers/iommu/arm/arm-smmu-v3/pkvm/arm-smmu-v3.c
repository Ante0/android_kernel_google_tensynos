...
static void smmu_telemetry_init_domain_data(pkvm_handle_t domain_id)
{
...
static void smmu_telemetry_add_prot_mem_size(size_t size)
{
	if (!smmu_telemetry_is_enable())
		return;

	kvm_hyp_shared_arm_smmu_telemetry->hs2t.prot_mem_usage += size;
	kvm_hyp_shared_arm_smmu_telemetry->hs2t.max_prot_mem_usage =
		max(kvm_hyp_shared_arm_smmu_telemetry->hs2t.prot_mem_usage,
		    kvm_hyp_shared_arm_smmu_telemetry->hs2t.max_prot_mem_usage);
}

static void smmu_telemetry_sub_prot_mem_size(size_t size)
{
	if (!smmu_telemetry_is_enable())
		return;

	if (size > kvm_hyp_shared_arm_smmu_telemetry->hs2t.prot_mem_usage)
		kvm_hyp_shared_arm_smmu_telemetry->hs2t.prot_mem_usage = 0;
	else
		kvm_hyp_shared_arm_smmu_telemetry->hs2t.prot_mem_usage -= size;
}
				     ...
				     phys_addr_t start, phys_addr_t end, int prot)
{
	...
	if (prot) {
		...
		while (size) {
			...
			start += mapped;
			if (mapped)
				smmu_telemetry_sub_prot_mem_size(mapped);
			if (!mapped || ret)
				return;
		}
	} else {
		while (size) {
			...
			smmu_telemetry_add_prot_mem_size(unmapped);
		}
	}
}
